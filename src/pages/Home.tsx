...
export const Home = () => {
	...
	const getSceneOpacity = (i: number): number => {
		...
		if (scrollY >= start && scrollY < fadeOutStart) return 1;
	...
	const s1 = useSpring({
		...
		filter: getSceneFilter(0),
		config: sc,
	});
	const s2 = useSpring({
		opacity: getSceneOpacity(1),
		filter: getSceneFilter(1),
		config: sc,
	});
	const s3 = useSpring({
		opacity: getSceneOpacity(2),
		filter: getSceneFilter(2),
		config: sc,
	});
	const s4 = useSpring({
		opacity: getSceneOpacity(3),
		filter: getSceneFilter(3),
		config: sc,
	});
	...
	const arrowOpacity =
		sceneIdx < totalScenes - 1
			? within < fadeStart
				? 1
				: clamp(1 - (within - fadeStart) / (vh - fadeStart), 0, 1)
			: 0;
		...
		<>
			...
			<Canvas
				style={{
					...
					zIndex: 0,
					background:
						"linear-gradient(to bottom, #130826 0%, #0C051C 25%, #070318 50%, #050A18 100%)",
				}}
